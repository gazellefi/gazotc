  ...
  <div class="bblist">
	  ...
	  <div class="hidden-xs-only">
	  	<div class="bblist_table">
			...
			<el-select size="mini" v-model="fabi" placeholder="Please select">
			  <el-option v-for="item in fbarr" :key="item.key" :label="item.id" :value="item.key">
			  </el-option>
			</el-select>
	  	
	  	    <el-input size="mini" v-model="form.zdnum" :placeholder="$t('message.enterContent')" style="margin-left: 15px; max-width: 100px"></el-input>
	  	
	  	    <el-button size="mini" type="primary" icon="el-icon-search" circle style="margin-left: 15px" @click="listajax"></el-button>
	  	  </div>
	  	  <el-table v-loading="jiazai" :data="list">
	  	    <el-table-column align=center width=80px prop="iorder" :label="$t('message.arbitration.order')"> </el-table-column>
	  	    ...
	  	    <el-table-column align=center width=80px prop="uni" :label="$t('message.dapp.unitPrice')"> </el-table-column>
	  	    <el-table-column width=250px :label="$t('message.operation')">
	  	      <template slot-scope="scope">
	  	        <el-button type="text" size="small" @click="opengoumai(scope.row)">{{ $t("message.dapp.buy")}}</el-button>
	  	      </template>
	  	    </el-table-column>
	  	  </el-table>
	  	</div>
	  ...
	  <div class="hidden-sm-and-up">
		  <el-row style="border-bottom: 1px solid #EEEEEE;">
		  	...
		  	<el-col :span="12">
		  		<span class="typeSelect">{{ $t("message.dapp.unit")}}：</span>
		  		<el-select size="mini" placeholder="Please select" style="margin-left: 15px;width: 50%;" v-model="form.type_b">
		  		  <el-option v-for="item in hbarr" :key="item.key" :label="item.id" :value="item.key">
		  		  </el-option>
		  		</el-select>
		  	</el-col>
		  	<el-col :span="12" style="margin-top: 20px;padding-bottom: 10px;">
		  		<div class="ddgl_fabu_p_head_sxan_i">
		  			<el-input size="mini" v-model="form.zdnum" :placeholder="$t('message.enterContent')" style="max-width: 100px"></el-input>
					<el-button size="mini" type="primary" icon="el-icon-search" circle style="margin-left: 15px;" @click="listajax"></el-button>
		  		</div>
		  	</el-col>
		  </el-row>
		  ...
		  <div class="list_nav">
			  <div class="list_item f a_e c_b" v-for="(item,index) in list" :key="index">
			  	...
			  	<div>
					<el-button plain size="mini" @click="opengoumai(item)" style="border-color: #FDC500;color: #FDC500;">
					{{ $t("message.dapp.buy")}}
					</el-button>
				</div>
			  </div>
