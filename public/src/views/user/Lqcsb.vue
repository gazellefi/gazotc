...
<template>
  <div class="lqcsb">
    <van-nav-bar class="lqcsb_w_top" title="Receive test currency" left-arrow :fixed="true" :placeholder="true" z-index="55" :safe-area-inset-top="true" @click-left="goback" />

    <div class="lqcsb_w" v-if="key == 'huobi'">
      <div class="lqcsb_w_ul">
        <div class="lqcsb_w_ul_t">Please select an asset</div>
        <div class="lqcsb_w_ul_item">
          <div class="lqcsb_w_ul_item_l">
            <el-select v-model="zckey" placeholder="Please select">
              <el-option v-for="item in zclisthb" :key="item['id']" :label="item['id']" :value="item['heyue']">
              </el-option>
            </el-select>
          </div>
          <div class="lqcsb_w_ul_item_r">
            <van-button type="primary" @click="lqajax" :loading="jiazai">receive</van-button>
          </div>
        </div>
        <div class="lqcsb_w_ul_item temsg">
          If receiving test currency fails<br /> Please contact wechat: lusun2016
        </div>
        <div class="qianbao_view_head_item_li" style="opacity: 1.5;">Your test currency balance(Test)：</div>
    ...
    <div class="lqcsb_w" v-if="key != 'huobi'">
      <div class="lqcsb_w_ul">
        ...
        <div class="lqcsb_w_ul_item">
          <div class="lqcsb_w_ul_item_l">
            <el-select v-model="zckey" placeholder="Please select">
              <el-option v-for="item in zclist" :key="item['id']" :label="item['id']" :value="item['key']">
              </el-option>
            </el-select>
          </div>
          <div class="lqcsb_w_ul_item_r">
        ...
        <div class="lqcsb_w_ul_item temsg">
          To receive the test currency, you need to have Ethereum rinkeby test network eth in your account!<br /> If not, please contact wechat: lulusun2016
        </div>
      </div>
    </div>
  </div>
    ...
    lqajax() {
      ...
      async function ispan_time() {
        ...
        if (timestamp - time >= 3600) {
          ...
          Dialog.alert({
            title: 'Tips',
            message: 'You have received it!, Please in' + (fen / 60) + 'Pick it up again in minutes!',
          });
        }
      }
      ...
      function lqzicanajax() {
        lquconn.methods.faucet(dq.zckey + "").send({
          ...
          dq.jiazai = false;
          if (ret) {
            var msg = knum + ktitle + 'Sent to your account!';
            Dialog.alert({
              title: 'Tips',
              message: msg,
            });
          } else {
            Notify({ type: 'warning', message: 'Please agree to authorize!' });
          }
        });
      }
    }
